...
//  Smartpress
//
//  Created by Paul Max on 11/20/20.
...
class StreamCell: UITableViewCell {
    ...
    public func cellConfigure(stream: [StreamModel], for indexPath: IndexPath) {
        ...
        if stream[indexPath.row].date == nil {
            dateLabel.text = stream[indexPath.row].position
            ...
            dateLabel.text = String((stream[indexPath.row].date)!.dropLast(3))
        ...
        if indexPath.row == stream.lastIndex(where: { (stream) -> Bool in
            return true
        }) {
...
extension StreamCell {
    ...
    private func customLayoutSubviews() {
        ...
        photoImageView.widthAnchor.constraint(equalToConstant: contentView.width / 2).isActive = true
        ...
        titleLabel.topAnchor.constraint(equalTo: contentView.topAnchor, constant: SPACE).isActive = true
        ...
        dateLabel.centerYAnchor.constraint(equalTo: photoImageView.centerYAnchor).isActive = true
        dateLabel.heightAnchor.constraint(lessThanOrEqualToConstant: 60.0).isActive = true
        dateLabel.leftAnchor.constraint(equalTo: photoImageView.rightAnchor, constant: SPACE).isActive = true
        dateLabel.rightAnchor.constraint(equalTo: contentView.rightAnchor, constant: -SPACE).isActive = true
