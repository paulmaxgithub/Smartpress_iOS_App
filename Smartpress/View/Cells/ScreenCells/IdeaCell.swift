...
class IdeaCell: UITableViewCell {
    ...
    public func cellConfigure(idea: [IdeaModel], for indexPath: IndexPath) {
        
        dateLabel.text = String((idea[indexPath.row].date)!.dropLast(6))
        tagsLabel.text = "#\((idea[indexPath.row].tags).joined(separator: " #"))"
        allTextLabel.text = idea[indexPath.row].title
        
        // Getting Photo Data from JSON
        guard let url = URL(
            string: WebAddressURL.url + (idea[indexPath.row].picture!)) else {
                return
        }
        ...
        if indexPath.row == idea.lastIndex(where: { (idea) -> Bool in
            return true
        }) {
